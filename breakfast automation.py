...
# Raw GitHub URL for shared storage — fetched at runtime, no redeploy needed to pick up updates
STORAGE_URL = os.getenv(
    'STORAGE_URL',
    'https://raw.githubusercontent.com/mamamancunian/breakfast_box_recipes/main/breakfast_storage.json'
)

# How often to re-fetch storage from GitHub (seconds)
STORAGE_REFRESH_INTERVAL = 300  # 5 minutes
...
class BreakfastReminder:
    def __init__(self):
        ...
        self.inventory = {}
        self.week_plan = {}
        self.last_storage_fetch = None
        self.load_storage()

    def load_storage(self):
        """Fetch recipes, inventory and week plan from GitHub raw URL (cache-busted)."""
        try:
            # Cache-bust with a timestamp query param so GitHub/CDN doesn't serve stale content
            url = f"{STORAGE_URL}?t={int(time.time())}"
            ...
            if response.status_code == 200:
                ...
                self.last_storage_fetch = now_berlin()
                print(f"Storage refreshed OK at {self.last_storage_fetch.isoformat()} "
                      f"({len(self.recipes)} recipes)", flush=True)
            else:
                print(f"Storage fetch failed: HTTP {response.status_code}", flush=True)
        except Exception as e:
            print(f"Error fetching storage: {e}", flush=True)

    def maybe_refresh_storage(self):
        """Re-fetch storage periodically so GitHub updates are picked up without a redeploy."""
        if self.last_storage_fetch is None:
            self.load_storage()
            return
        elapsed = (now_berlin() - self.last_storage_fetch).total_seconds()
...
if __name__ == "__main__":
    ...
    print(f"Storage URL: {STORAGE_URL}", flush=True)
